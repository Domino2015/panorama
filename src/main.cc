...
//
/*目录结构
 * feature/   Lowe的SIFT算法实现
 * lib/imgproc.cc  仿射变换矩阵，通过直接线性变换(Direct Linear Transform)估计 变换矩阵
 * stitch/transform_estimate.cc   RANSAC剔除噪声。
 * stitch/homography.hh  优劣检查(health check)，以避免因为错误的匹配对而产生畸变的变换估计
 * stitch/cylstitcher.cc  柱面投影去畸变图片
 * stitch/warp.cc   柱面投影
 * stitch/camera_estimate.cc   粗略估计所有相机焦距的方法（通过捆绑调整被全局矫正）
...
#define CFG(x) \
	x = Config.get(#x)
	const char* config_file = "/Users/wangwei/CLionProjects/OpenPano/src/config.cfg";
